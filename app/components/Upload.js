...
export default class Upload extends React.Component {
  ...
  deleteYes() {
    ...
    if (delete_id == 'all') {
      S3UploaderModelOperations.getAllFiles().then(files => {
        ...
        if (fileNamesObj.length) {
          ...
          const axiosConfigObject = {
            ...
            data: { file_names: fileNamesObj }
          };
          axios
            .delete(deleteUrl, axiosConfigObject)
            .then(response => {
              ...
              if (response.data.connectivity) {
                ...
                ToastStore.error(
                  response.data.connectivityProb,
                  this.toast_timeout,
                  'custom-toast-error'
                );
      ...
      S3UploaderModelOperations.getFileInfoById(delete_id).then(dbResp => {
        ...
        const axiosConfigObject = {
          headers: httpReqHeaders,
          data: { file_name: dbResp.dataValues.file_name }
        };
        axios
          .delete(deleteUrl, axiosConfigObject)
          .then(response => {
            if (response.data.connectivity) {
              S3UploaderModelOperations.deleteFile(delete_id).then(
                deleteResp => {
              ...
              S3UploaderModelOperations.updateFile(deleteObj).then(
                deleteResp => {
                  this.state.listOfFiles.splice(rowIndex, 1);
                  this.setState({ listOfFiles: this.state.listOfFiles });
                  ToastStore.success(
            ...
            if (typeof response.data.notFoundOnLocalDisk != 'undefined') {
              ToastStore.error(
                response.data.notFoundOnLocalDisk,
                this.toast_timeout,
                'custom-toast-error'
              );
            }
    ...
    axios
      ...
      .then(() => {
        setTimeout(() => {
          ...
          this.setState({ showSpinner: false });
          ToastStore.success(
            '50 Test Images are uploaded to S3',
            this.toast_timeout,
            'custom-toast-success'
          );
        }, 100);
  ...
  downloadFile(file, s3_url) {
    ...
    axios({
      ...
      if (response.data.size <= 47) {
        ToastStore.error(
          'This file was not found in local disk',
          this.toast_timeout,
          'custom-toast-error'
        );
      } else {
  ...
  getFilesData() {
    S3UploaderModelOperations.createModalAndGetAllFiles().then(files => {
      files.forEach(item => {
            ...
            if (
              ...
              S3UploaderModelOperations.newFile(model).then(dbResp => {
                if (dbResp) {
                  ...
                  modelObj.uploaded_on = this.getLocalTime(
                    dbResp.dataValues.uploaded_on
                  );
    ...
    return (
      <div className={styles.container} data-tid="container">
        <Modal
          ...
          toggle={this.openCModal}
          className={this.props.className}
        >
          <ModalHeader toggle={this.openCModal}>Delete ?</ModalHeader>
          <ModalBody>{this.state.deleteText}</ModalBody>
          <ModalFooter>
            <Button color="primary" onClick={this.deleteYes}>
              YES
            </Button>{' '}
            <Button color="secondary" onClick={this.deleteNo}>
              NO
            </Button>
          </ModalFooter>
        </Modal>
            ...
            <table className="modalTable">
              <tbody>
                ...
                <tr>
                  <th>Connectivity</th>
                  <td>
                    {this.state.file_info.connectivity ? 'ONLINE' : 'OFFLINE'}
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <form className="uploader-form" onSubmit={this.onFormSubmit}>
          <h3 className="text-center">
            <i className="fas fa-cloud-upload-alt"></i> S3 Uploader
          </h3>
          <div className="upload-input-wrapper">
            <div className="upload-input">
              <input
                className="uploadable-input d-none"
                id="uploadLocalFile"
                name="uploadLocalFile"
                type="file"
                onChange={this.onChange}
                ref="uploadFile"
                accept="application/msword, application/pdf, image/*, application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                multiple
              />
              <label htmlFor="uploadLocalFile">
                {this.state.fileName
                  ? this.state.fileName
                    ...
                    Cell: row => (
                      <div>
                        <span
                          title="Update"
                          onClick={() =>
                            this.selectUpdateFile(row.original._id, row.index)
                          }
                        >
                          <i className="fas fa-upload hover"></i>
                        </span>
                        <span> / </span>
                        <span
                          title="Delete"
                          onClick={() =>
                            this.deleteFile(row.original._id, row.index)
                          }
                        >
                          <i className="fas fa-trash hover"></i>
                        </span>
                        <span> / </span>
                        <span
                          title="Download"
                        ...
                        >
                          <i className="fas fa-file-download hover"></i>
                        </span>
                      </div>
                    ),
                    minWidth: 20
